...
DEMO_NAME="ARIES AI Assistant Demo"
DEMO_DIR_NAME="aries-ai-assistant-demo"
...
pushd backend >/dev/null
  ...
  for model in "${STT_MODELS[@]}"; do
    run_as_user uv run python -c "from huggingface_hub import snapshot_download; snapshot_download('${model}')"
  done
...
if [ -f /usr/share/applications/ai-assistant-demo.desktop ]; then
  sudo rm /usr/share/applications/ai-assistant-demo.desktop
fi
